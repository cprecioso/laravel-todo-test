...
class ProjectInviteController extends Controller
{
    ...
    public function sendInviteEmail(
        ...
        Mail::to($email)->queue(
            new InvitedToProject(Auth::user(), $project, $url),
        );
    }
    ...
    public function accept(Request $request, ProjectInvite $projectInvite)
    {
        ...
        ProjectInvite::where("email", "=", $projectInvite->email)->each(
            function ($invite) {
        ...
        return redirect()->route("project", [
            "project" => $projectInvite->project()->first()->id,
        ]);
    }
    ...
    public static function cleanupOldInvites()
    {
        ...
        $deleteCount = 0;
        ProjectInvite::where("expires_at", "<", now())->each(function (
            $invite,
        ) use (&$deleteCount) {
            $invite->delete();
            $deleteCount++;
        ...
        Log::info(
            "Finished cleanupOldInvites task, deleted " .
                $deleteCount .
                " invites",
        );
    }
